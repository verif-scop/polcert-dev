...
| Efield (l, f, _) ->
  bind2
    (match typeof l with
     | Tstruct (id, _) ->
       bind (lookup_composite ce id) (fun co ->
         field_offset ce f co.co_members)
     | Tunion (id, _) ->
       bind (lookup_composite ce id) (fun co ->
         union_field_offset ce f co.co_members)
     | _ ->
       Error
         (msg
           ('i'::('l'::('l'::('-'::('t'::('y'::('p'::('e'::('d'::(' '::('f'::('i'::('e'::('l'::('d'::(' '::('a'::('c'::('c'::('e'::('s'::('s'::[]))))))))))))))))))))))))
    (fun delta bf ->
    bind (constval ce l) (fun v ->
      match bf with
      | Full ->
        OK
          (if ptr64
...
| Ecast (r, ty) -> bind (constval ce r) (fun v1 -> do_cast v1 (typeof r) ty)
| Eseqand (r1, r2, _) ->
  bind (constval ce r1) (fun v1 ->
    bind (constval ce r2) (fun v2 ->
      match bool_val v1 (typeof r1) Mem.empty with
      | Some b ->
        if b then do_cast v2 (typeof r2) type_bool else OK (Vint Int.zero)
      | None ->
        Error
          (msg
...
| Eseqor (r1, r2, _) ->
  bind (constval ce r1) (fun v1 ->
    bind (constval ce r2) (fun v2 ->
      match bool_val v1 (typeof r1) Mem.empty with
      | Some b ->
        if b then OK (Vint Int.one) else do_cast v2 (typeof r2) type_bool
      | None ->
        Error
          (msg
            ('u'::('n'::('d'::('e'::('f'::('i'::('n'::('e'::('d'::(' '::('|'::('|'::(' '::('o'::('p'::('e'::('r'::('a'::('t'::('i'::('o'::('n'::[])))))))))))))))))))))))))
| Econdition (r1, r2, r3, ty) ->
  bind (constval ce r1) (fun v1 ->
    bind (constval ce r2) (fun v2 ->
      bind (constval ce r3) (fun v3 ->
        match bool_val v1 (typeof r1) Mem.empty with
        | Some b ->
          if b then do_cast v2 (typeof r2) ty else do_cast v3 (typeof r3) ty
        | None ->
          Error
            (msg
...
| Ecomma (r1, r2, _) -> bind (constval ce r1) (fun _ -> constval ce r2)
| Eparen (r, tycast, _) ->
  bind (constval ce r) (fun v -> do_cast v (typeof r) tycast)
| _ ->
  Error
    (msg
...
let constval_cast ce a ty =
  bind (constval ce a) (fun v -> do_cast v (typeof a) ty)

type state = { init : init_data list; curr : coq_Z; total_size : coq_Z }

(** val init : state -> init_data list **)

let init s =
  s.init

(** val curr : state -> coq_Z **)

let curr s =
  s.curr

(** val total_size : state -> coq_Z **)
...
let rec normalize il depth =
  ...
  then OK il
        ...
        | i :: il0 ->
          (match i with
           | Init_int8 n ->
             bind (normalize il0 (Z.sub depth (Zpos Coq_xH))) (fun il' -> OK
               ((Init_int8 n) :: il'))
           | Init_int16 n ->
             bind (normalize il0 (Z.sub depth (Zpos (Coq_xO Coq_xH))))
               (fun il' -> OK
               (add_rev_bytes (encode_int (S (S O)) (Int.unsigned n)) il'))
           | Init_int32 n ->
             bind
               (normalize il0 (Z.sub depth (Zpos (Coq_xO (Coq_xO Coq_xH)))))
               (fun il' -> OK
               (add_rev_bytes (encode_int (S (S (S (S O)))) (Int.unsigned n))
                 il'))
           | Init_int64 n ->
             bind
               (normalize il0
                 (Z.sub depth (Zpos (Coq_xO (Coq_xO (Coq_xO Coq_xH))))))
               (fun il' -> OK
               (add_rev_bytes
             ...
             bind
               ...
               (add_rev_bytes
                 (encode_int (S (S (S (S O))))
                   (Int.unsigned (Float32.to_bits f))) il'))
           | Init_float64 f ->
             bind
               (normalize il0
                 (Z.sub depth (Zpos (Coq_xO (Coq_xO (Coq_xO Coq_xH))))))
               ...
               (add_rev_bytes
                 (encode_int (S (S (S (S (S (S (S (S O))))))))
                   (Int64.unsigned (Float.to_bits f))) il'))
           | Init_space n ->
             let n0 = Z.max Z0 n in
             if zle n0 depth
             then bind (normalize il0 (Z.sub depth n0)) (fun il' -> OK
                    (add_zeros n0 il'))
...
let trisection il depth sz =
  bind (normalize il (Z.add depth sz)) (fun il0 ->
    bind2 (decompose il0 depth) (fun bytes1 il1 ->
      bind2 (decompose il1 sz) (fun bytes2 il2 -> OK ((bytes1, bytes2), il2))))
...
let store_data s pos i =
  ...
  then if zle s.curr pos
       then OK { init =
            ...
            bind (trisection s'.init (Z.sub s'.curr (Z.add pos sz)) sz)
              (fun x3 ->
              let (p, il2) = x3 in
              let (bytes1, _) = p in
...
let load_int s pos isz =
  ...
  then let s' = pad_to s (Z.add pos sz) in
       bind (trisection s'.init (Z.sub s'.curr (Z.add pos sz)) sz) (fun x3 ->
         let (p, _) = x3 in
...
(** val initializer_rect :
    (expr -> 'a1) -> (initializer_list -> 'a1) -> (initializer_list -> 'a1)
    -> (ident -> coq_initializer -> 'a1 -> 'a1) -> coq_initializer -> 'a1 **)

let rec initializer_rect f f0 f1 f2 = function
| Init_single a -> f a
| Init_array il -> f0 il
| Init_struct il -> f1 il
| Init_union (f3, i0) -> f2 f3 i0 (initializer_rect f f0 f1 f2 i0)

(** val initializer_rec :
    (expr -> 'a1) -> (initializer_list -> 'a1) -> (initializer_list -> 'a1)
    -> (ident -> coq_initializer -> 'a1 -> 'a1) -> coq_initializer -> 'a1 **)

let rec initializer_rec f f0 f1 f2 = function
| Init_single a -> f a
| Init_array il -> f0 il
| Init_struct il -> f1 il
| Init_union (f3, i0) -> f2 f3 i0 (initializer_rec f f0 f1 f2 i0)

(** val initializer_list_rect :
    'a1 -> (coq_initializer -> initializer_list -> 'a1 -> 'a1) ->
    initializer_list -> 'a1 **)

let rec initializer_list_rect f f0 = function
| Init_nil -> f
| Init_cons (i0, il) -> f0 i0 il (initializer_list_rect f f0 il)

(** val initializer_list_rec :
    'a1 -> (coq_initializer -> initializer_list -> 'a1 -> 'a1) ->
    initializer_list -> 'a1 **)

let rec initializer_list_rec f f0 = function
| Init_nil -> f
| Init_cons (i0, il) -> f0 i0 il (initializer_list_rec f f0 il)
...
let transl_init_single ce ty a =
  bind (constval_cast ce a ty) (fun v ->
    match v with
    | Vundef ->
      Error
        (msg
...
let rec transl_init_rec ce s ty i pos =
  match i with
  | Init_single a ->
    bind (transl_init_single ce ty a) (fun d -> store_data s pos d)
  | Init_array il ->
    (match ty with
     | Tarray (tyelt, nelt, _) ->
  ...
  | Init_struct il ->
    (match ty with
     | Tstruct (id, _) ->
       bind (lookup_composite ce id) (fun co ->
         match co.co_su with
         | Struct -> transl_init_struct ce s co.co_members il pos Z0
         | Union ->
           Error ((MSG
       ...
       bind (lookup_composite ce id) (fun co ->
         ...
         | Struct ->
           Error ((MSG
             ...
             id) :: []))
         | Union ->
           bind (field_type f co.co_members) (fun ty1 ->
             bind2 (union_field_offset ce f co.co_members)
               (fun delta layout ->
               match layout with
               | Full -> transl_init_rec ce s ty1 i1 (Z.add pos delta)
               | Bits (sz, _, p, w) ->
                 transl_init_bitfield ce s ty1 sz p w i1 (Z.add pos delta))))
     | _ ->
       Error
         (msg
           ('w'::('r'::('o'::('n'::('g'::(' '::('t'::('y'::('p'::('e'::(' '::('f'::('o'::('r'::(' '::('c'::('o'::('m'::('p'::('o'::('u'::('n'::('d'::(' '::('i'::('n'::('i'::('t'::('i'::('a'::('l'::('i'::('z'::('e'::('r'::[])))))))))))))))))))))))))))))))))))))
...
and transl_init_array ce s tyelt il pos =
  ...
  | Init_nil -> OK s
  | Init_cons (i1, il') ->
    bind (transl_init_rec ce s tyelt i1 pos) (fun s1 ->
      transl_init_array ce s1 tyelt il' (Z.add pos (sizeof ce tyelt)))
  ...
  | Init_cons (i1, il') ->
    let rec init0 ms0 pos0 =
      ...
      | m :: ms' ->
        ...
        else bind2 (layout_field ce pos0 m) (fun delta layout ->
               bind
                 (match layout with
                  | Full ->
                    transl_init_rec ce s (type_member m) i1 (Z.add base delta)
                  | Bits (sz, _, p, w) ->
                    transl_init_bitfield ce s (type_member m) sz p w i1
                      (Z.add base delta)) (fun s1 ->
                 transl_init_struct ce s1 ms' il' base (next_field ce pos0 m)))
    in init0 ms pos
...
let transl_init ce ty i =
  ...
  bind (transl_init_rec ce s0 ty i Z0) init_data_list_of_state
