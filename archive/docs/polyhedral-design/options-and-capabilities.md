...
| 选项 | 模块 | 当前 artifact | 效果类别 | 说明 |
| --- | --- | --- | --- | --- |
| `--dfp` | `lib/framework-dfp.c` | build-dependent | `S0` | 另一套 affine hyperplane 合成框架；若 Pluto binary 支持，PolOpt 可把它当 affine oracle tuning |
| `--typedfuse` | `main.cpp`, `framework-dfp.c` | build-dependent | `S0` | 依赖 DFP + solver；验证边界仍是 affine output validation |
| `--hybridfuse` | `main.cpp`, `framework-dfp.c` | build-dependent | `S0` | 外层 typed，内层 max；先作为 oracle tuning 处理 |
| `--delayedcut` | `main.cpp`, `framework-dfp.c` | build-dependent | `S0` | DFP 中延迟切分；不应影响 trusted checker |
| `--lp`, `--ilp`, `--glpk`, `--gurobi`, `--lpcolor`, `--clusterscc` | `main.cpp`, `framework-dfp.c`, solver backend | build-dependent | `S0` | solver / colouring 选择；若库存在可以 pass through，再由 affine validator 检查结果 |

### 对 validation 的含义

DFP 在语义上仍属于“找 affine hyperplane”的范畴，本身不要求 domain mutation。因此它并不是最难验证的一支。真正风险是：

- 具体 Pluto binary 是否带对应 LP/ILP solver
- 它引入了另一套调度搜索逻辑和 colouring witness，可能暴露当前 import/canonicalization 没覆盖过的 schedule shape

如果只把 DFP 当 untrusted affine oracle tuning，正确性仍由现有 affine
validator 兜底。是否要证明 Pluto DFP 决策过程本身，是另一件事。
...
| 选项 | 模块 | 当前 artifact | 效果类别 | 说明 |
| --- | --- | --- | --- | --- |
| `--prevector` / `--noprevector` | `post_transform.c`, `ast_transform.c` | Pluto 可用，PolOpt 有 checked annotation route | `C1` | Pluto vector bit 只作为候选 hint；PolOpt 重新用 doall/parallel certificate 认证 |
| `--unrolljam` / `--nounrolljam` | `polyloop.c`, `ast_transform.c` | Pluto 可用，PolOpt 有 checked Loop-level subset | `C1` | Pluto 候选选择不可信；PolOpt 应用自己的 checked unroll/jam pass |
| `--ufactor` | `polyloop.c`, `ast_transform.c` | 可用；PolOpt 在 checked subset 中接受 | `C1` | unroll factor，必须通过 checked pass 的 shape 条件 |
...
`prevector` 不完全是纯 codegen 选项，因为它也会参与 point-loop 选择启发式；
但 PolOpt 当前只接受 annotation-only 解释：Pluto hint 决定候选优先级，
不决定正确性。`unrolljam` 也不是 pass-through；当前支持的是 PolOpt 自己
的 checked Loop-level subset。
...
1. 当前已覆盖或接近可覆盖的 state-preserving optimizer surface
   - affine scheduling and fusion knobs such as `smartfuse`, `nofuse`,
     `maxfuse`, `rar`
   - `tile`, `second-level-tile`, `iss`, `diamond-tile`,
     `full-diamond-tile`
   - checked `parallel`, `parallel-current`, checked `multipar` as a separate
     route
   - checked `prevector` annotation and checked Loop-level `unrolljam`
2. 适合作为 oracle tuning pass-through 的选项
   - dependence / solver / DFP knobs such as `candldep`, `lastwriter`, `dfp`,
     `typedfuse`, `lp`, `glpk`, when the Pluto build supports them
3. 需要单独语义层的选项
   - `scalpriv`, storage contraction, layout remapping, overlapped tiling, and
     other transformations that allocate or remap storage
4. 仍应留在 PolOpt optimizer surface 外面的选项
   - frontend choice (`pet`, arbitrary `readscop` as normal source input) and
     pure backend formatting / CLooG-output flags
