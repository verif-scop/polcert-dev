...
| 文件 | 作用 |
| --- | --- |
| `/pluto/lib/pluto.c` | Pluto 核心调度器；hyperplane 搜索；cut heuristic；diamond-aware hyperplane 选择 / replacement 的 classic 入口 |
...
| `/pluto/lib/tile.c` | ordinary tiling、tile schedule / wavefront schedule；在 diamond 情况下作用于前一阶段已经改写过的 band |
